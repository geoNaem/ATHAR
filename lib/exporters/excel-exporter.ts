...
export async function buildExcelExport(
  ...
  result.themes.forEach((theme, i) => {
    ...
    let strengthVal = indicator?.evidence_strength || '';
    if (isAr && strengthVal) {
      if (strengthVal === 'Strong') strengthVal = L!.strong;
      if (strengthVal === 'Moderate') strengthVal = L!.moderate;
      if (strengthVal === 'Weak') strengthVal = L!.weak;
      if (strengthVal === 'Not found') strengthVal = L!.notFound;
    }
  ...
  if (result.logframe_alignment.length === 0) {
    ...
    result.logframe_alignment.forEach((entry, i) => {
      ...
      let s = entry.evidence_strength;
      if (isAr) {
         if (s === 'Strong') s = L!.strong;
         if (s === 'Moderate') s = L!.moderate;
         if (s === 'Weak') s = L!.weak;
         if (s === 'Not found') s = L!.notFound;
      }
