...
export async function buildWordExport(
  ...
  const bodyContent = structureId === 'thematic'
    ? buildThematicSections(result, preset)
    : buildIndicatorSections(result, preset);

  // Apply RTL/Arabic labels to bodyContent if necessary
  if (isAr) {
    bodyContent.forEach(p => {
       // All paragraphs in AR mode should be bidirectionally handled
       // We'll perform labels mapping in the structural files if specified,
       // but here we ensure the container paragraph properties are correct.
       p.options.bidirectional = true;
       p.options.alignment = AlignmentType.RIGHT;
    });
  }
