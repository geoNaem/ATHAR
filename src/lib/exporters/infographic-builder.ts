/**
 * Data transformation layer for the PDF infographic.
 * Extracts and normalizes AnalysisResult into a flat, renderable InfographicData shape.
 */
...
export function prepareInfographicData(result: AnalysisResult): InfographicData {
  const sorted = [...result.themes].sort((a, b) => b.frequency - a.frequency);
  const top5 = sorted.slice(0, 5);
  const maxFreq = top5[0]?.frequency ?? 1;

  const pullQuote =
    (result.themes[0]?.quotes[0] ?? result.key_findings[0] ?? '').slice(0, 180);
  ...
  return {
    ...
    top4findings: result.key_findings.slice(0, 4).map(f =>
